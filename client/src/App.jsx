...
export default function Game() {
	...
	function onSelect(char) {
		const answer = Ans.find((c) => c.name === char.name);
		if (!answer) return;
		const dx = Math.abs(answer.coords.x - box.xPct * 100);
		const dy = Math.abs(answer.coords.y - box.yPct * 100);
		if (dx < 3 && dy < 3) {
			setResult(
			...
			toast.success(<img src={char.icon} className="size-7"></img>);
			...
			toast.error(<img src={char.icon} className="size-7"></img>);
		}
